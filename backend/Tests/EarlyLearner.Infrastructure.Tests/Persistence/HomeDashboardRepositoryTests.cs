...
public sealed class HomeDashboardRepositoryTests : BaseDatabaseSetup
{
    ...
    public async Task GetAsync_Should_ReturnDashboardForCurrentHousehold()
    {
        ...
        var touchedOutcome = LearningSeeder.CreateOutcome("language-listening", "Listens and responds", "Listens to instructions.", "Language", 10);
        var untouchedOutcome = LearningSeeder.CreateOutcome("social-turn-taking", "Takes turns with others", "Practices waiting.", "Social", 20);
        var inactiveOutcome = LearningSeeder.CreateOutcome("archived-outcome", "Archived outcome", "Old outcome.", "Archive", 30);
        ...
        var otherLog = LearningSeeder.CreateDailyLog(otherSeed.Household, otherChild, today, LearningMomentKindEnum.Activity, "Other activity", "Not visible.", touchedOutcome);
        await HouseholdSeeder.SeedAsync(Db, seed, otherSeed);
        await LearningSeeder.SeedOutcomesAsync(Db, touchedOutcome, untouchedOutcome, inactiveOutcome);
