...
public sealed record CreateGoalCommand(Guid HouseholdId, Guid ChildId, string Title, GoalTypeEnum Type, DateOnly StartDate, DateOnly EndDate, IReadOnlyList<Guid> ReadinessOutcomeIds);
...
public sealed class GoalCommandService(IGoalCommandRepository goalRepo, IUnitOfWork uow) : IGoalCommandService
{
    public async Task<Result<GoalResponse>> CreateAsync(CreateGoalCommand command, CancellationToken cancellationToken)
    {
        var childExists = await goalRepo.ChildExistsAsync(command.HouseholdId, command.ChildId, cancellationToken);
        ...
        var goal = Goal.Create(new HouseholdId(command.HouseholdId), new ChildId(command.ChildId), command.Title, command.Type, timeframe, readinessOutcomes);
