  ...
  return (
    <section aria-label="Home quick actions" className="grid gap-3 sm:grid-cols-3">
      {quickActions.map((action) => (
        <Link
          className="inline-flex min-h-12 items-center justify-center gap-2 rounded-md bg-white px-4 text-sm font-semibold text-brand-heading shadow-app-card hover:text-brand-primary"
          key={action.label}
          to={action.to}
        >
          {action.icon}
