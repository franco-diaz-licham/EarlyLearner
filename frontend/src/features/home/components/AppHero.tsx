...
export const AppHero = ({ childName, currentUserName }: AppHeroProps) => {
  const greeting = currentUserName ? `Good morning, ${currentUserName}!` : 'Good morning!';
  const heroMessage = childName ? `${childName} is curious, creative and growing every day.` : 'Your learners are curious, creative and growing every day.';
  ...
  return (
    <section className="relative min-h-70 overflow-hidden rounded-md bg-white shadow-app-card">
      ...
      <div className="relative max-w-xl px-6 py-10 sm:px-10">
        ...
        <p className="mt-4 max-w-sm text-lg leading-8 text-brand-muted">{heroMessage}</p>
      </div>
    </section>
  );
};
